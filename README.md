...
our changes independently, with reproducible failures, regression tests and
explicit disclosure of AI assistance. This is a difference in contribution
policy, not a claim that SDL forbids independent forks or that an SDL maintainer
rejected a patch from us.

We respect SDL's contribution policy and will not submit prohibited AI-generated
code or comments upstream, or misrepresent their origin.
...
The initial Graphix change establishes the fork's identity and contribution
policy only. It does not yet fix the window-sizing defect, publish native
...
remain the build references. See [CONTRIBUTING.md](CONTRIBUTING.md) for Graphix's
own contribution policy. The upstream README is preserved in
